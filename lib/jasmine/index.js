...
var filename = __dirname + '/jasmine-1.0.0.js';
global.window = {
...
process.on('exit', function () {
  ...
  if (!jasmine.alreadyRan) {  
    jasmine.isVerbose = jasmine.isVerbose==undefined ? false : jasmine.isVerbose;
    jasmine.showColors =  jasmine.showColors==undefined ? true : jasmine.showColors;

    //need to set this or there's a VERY subtle "bug"...the test run just kind of stops.
    jasmine.getEnv().lastUpdate = new Date().getTime();
...
jasmine.execute = function(done){
  var log = [];
  var columnCounter = 0;
  var start = 0;
  var elapsed = 0;
  var verbose = jasmine.isVerbose || false;
  var colors = jasmine.showColors || false;
...
jasmine.requireAllSpecFiles = function(dir){
  files = jasmine.getAllSpecFiles(dir)
...
jasmine.getAllSpecFiles = function(dir){
  ...
  for (var i = 0, len = files.length; i < len; ++i){
    ...
    if (fs.statSync(filename).isFile() && 
        (filename.match(/_test\.js$/) || 
         filename.match(/_spec\.js$/) ||
