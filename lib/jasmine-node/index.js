...
jasmine.executeSpecsInFolder = function(options){
  ...
  var randomSeed =   options['randomSeed'];
  ...
  if (useRequireJs) {
    ...
    if (randomize) {
      var arrayShuffler = require('./array-shuffler');
      arrayShuffler.seedRandom(randomSeed);
      arrayShuffler.shuffle(specsList);

      if (isVerbose) {
        console.log('Executing tests in this order:');
        for( var i = 0; i < specsList.length; i++ ) {
          console.log( specsList[i].filename() );
        }
      }
    }
