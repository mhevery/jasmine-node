(function() {
  ...
  TerminalReporter = (function() {
    ...
    TerminalReporter.prototype.setColorFuncs = function(colorSet) {
      var color, func, _results;
      _results = [];
      for (color in colorSet) {
        func = colorSet[color];
        _results.push(this[color] = func);
      }
      return _results;
    };
    ...
    TerminalReporter.prototype.jasmineStarted = function(runner) {
      ...
      if (this.config.verbose) {
        ...
        this.config.print(this.stringWithColor(msg, this.pass));
      }
    ...
    TerminalReporter.prototype.reportUnfinished = function() {
      ...
      return this.config.print(this.stringWithColor(msg, this.fail));
    };
    ...
    TerminalReporter.prototype.printVerboseSuiteDone = function(suite) {
      ...
      msg += this.stringWithColor("" + suite.description + " Finish", this.ignore);
      msg += this.stringWithColor(" - " + this.times.suiteDone[suite.id] + " ms\n\n", this.suiteTiming);
      return this.config.print(msg);
    };
    ...
    TerminalReporter.prototype.makeSimple = function(spec) {
      ...
      switch (spec.status) {
        case 'pending':
          this.counts.skipped++;
          msg = this.stringWithColor('-', this.ignore);
          break;
        case 'passed':
          msg = this.stringWithColor('.', this.pass);
          break;
        case 'failed':
          this.counts.failures++;
          msg = this.stringWithColor('F', this.fail);
          break;
        default:
          msg = this.stringWithColor('U', this.fail);
      }
      return msg;
    };
    ...
    TerminalReporter.prototype.makeVerbose = function(spec) {
      ...
      switch (spec.status) {
        case 'pending':
          this.counts.skipped++;
          msg += this.stringWithColor("" + spec.description, this.ignore);
          break;
        case 'passed':
          msg += this.stringWithColor("" + spec.description, this.pass);
          break;
        case 'failed':
          this.counts.failures++;
          msg += this.stringWithColor("" + spec.description, this.fail);
          break;
        default:
          msg += this.stringWithColor("" + spec.description, this.fail);
      }
    ...
    TerminalReporter.prototype.printDoneFailures = function() {
      ...
      this.config.print(this.stringWithColor(msg, this.fail));
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        this.config.print(this.stringWithColor(msg, this.neutral));
      }
    };
      ...
      for (suite in _ref) {
        ...
        for (_i = 0, _len = specs.length; _i < _len; _i++) {
          ...
          for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
            failure = _ref1[_j];
            count++;
            this.config.print("\n\n" + indent + count + ") " + spec.fullName + "\n" + indent + indent + "Message:\n" + indent + indent + indent + (this.stringWithColor(failure.message, this.fail)) + "\n");
